...
export default class Main extends Phaser.Scene {
  ...
  update() {
    ...
    if (
      ...
      this.physics.moveTo(bullet, mouseX, mouseY, 200);
    ...
    this.bullet_group.children.iterate((bullet: any) => {
      //Doesnt seem right
      this.events.on("update", () => {
        if (
          bullet.active &&
          !this.physics.world.bounds.contains(bullet.x, bullet.y)
        ) {
          this.bullet_group.kill(bullet);
          bullet.destroy();
        }
      });
    });
